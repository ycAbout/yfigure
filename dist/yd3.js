(function (global, factory) {
  ...
  (global = global || self, factory(global.yd3 = {}));
  ...
  ******`;
  ...
  function bar(data, options = {}) {
    ...
    let yScale = linear$1()
      .domain([0, max(data.map((element) => element[yDataName]))])
      .range([innerHeight, 0]);
   ...
   */
  function histogram$1(data, options = {}) {
    //set up individual optional options so no need to feed options in a way non or all
    options.size ? true : options.size = { width: 400, height: 300 };
    options.margin ? true : options.margin = { left: 50, top: 20, right: 20, bottom: 50 };
    options.location ? true : options.location = 'body';
    ...
    let maxData = max(data, d => d[xDataName]);
    let minData = min(data, d => d[xDataName]);
    // X axis scale
    let xScale = linear$1()
    ...
    let histogram$1 = histogram()
      .value(d => d[xDataName])
      .domain(xScale.domain())
      .thresholds(thresholdArray); // split data into bins

    // to get the bins
    let bins = histogram$1(data);
  ...
  function lineDot(data, options = {}) {
    ...
    for (let j = 0; j < yDataNames.length; j++) {
      maxYArray.push(max(data, d => +d[yDataNames[j]]));  //parse float
      minYArray.push(min(data, d => +d[yDataNames[j]]));  //parse float
    }

    let ySetback = (max(maxYArray) - min(minYArray)) * 0.05;  //5% of data range

    let yScale = linear$1()
      .domain([min(minYArray) - ySetback, max(maxYArray)])  // data points off axis
      .range([innerHeight, 0]);

    //colors for difference lines
    let colorScale = ordinal()
      .domain(yDataNames)
  ...
  function scatter(data, options = {}) {
    ...
    let xMax = max(data, element => element[xDataName]);
    let xMin = min(data, element => element[xDataName]);
    ...
    let ySetback = (max(maxYArray) - min(minYArray)) * 0.02;  //2% of data range
  ...
  function sortableBar(data, options = {}) {
    ...
    let selection = select(options.location)
      .append('span')       //non-block container
    ...
    function draw(data, svg, order) {
      ...
      let yScale = linear$1()
        .domain([0, max(innerData.map((element) => element[yDataName]))])
        .range([innerHeight, 0]);
